...
module "rg_blue" {
  source = "./modules/rg" # (deploy && !green_golive) || (switch && green_golive) then rgignoreall else rg

  count = var.blue_deploy ? 1 : 0

  resource_group_name = "ch-demo-dev-euw-001-rg-blue"
  kubernetes_version  = local.kubernetes_version #(var.green_golive && var.deployment_phase == "deploy") || (!var.green_golive && var.deployment_phase == "switch") ? local.kubernetes_version : var.current_kubernetes_version

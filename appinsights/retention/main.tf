...
resource "null_resource" "log_analytics_table_retention" {
  ...
  provisioner "local-exec" {
    command = <<-SHELL
      ...
      az monitor log-analytics workspace table update --resource-group ${azurerm_resource_group.rg.name} --workspace-name ${azurerm_log_analytics_workspace.log_analytics.name} --name ${each.key} --retention-time ${local.application_insights_retention_days} --total-retention-time ${local.application_insights_retention_days} --subscription ${var.SUBSCRIPTIONID}
    SHELL
